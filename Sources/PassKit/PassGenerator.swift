...
public struct PassGenerator {
    ...
    public func copy(itemAt from: URL, as name: String? = nil) throws {
        let to = self.folder.appendingPathComponent(name ?? from.lastPathComponent)
    ...
    public func preparePass(pass: Pass) throws {
        ...
        let passUrl = self.folder.appendingPathComponent("pass.json")

        let write = self.fileManager.createFile(atPath: passUrl.path, contents: passData, attributes: nil)
        if !write {
    ...
    public func generateManifest() throws {
        ...
        let manifestUrl = self.folder.appendingPathComponent("manifest.json")

        let write = self.fileManager.createFile(atPath: manifestUrl.path, contents: manifestData, attributes: nil)
        if !write {
    ...
    public func generateSignature(certificate: URL, key: URL, wwdr: URL, password: String) throws {
        try shellOut(to: .generateSignature(
            ...
            key: key,
            wwdr: wwdr,
            manifest: self.folder.appendingPathComponent("manifest.json"),
            signature: self.folder.appendingPathComponent("signature"),
            password: password
        ))
    }
    ...
    public func zipPass(url: URL) throws {
        try self.fileManager.zipItem(at: self.folder, to: url, shouldKeepParent: false)
    }
